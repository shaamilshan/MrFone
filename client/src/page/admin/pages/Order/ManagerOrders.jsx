...
import { getManagerOrders, getOrders } from "../../../../redux/actions/admin/ordersAction";
...
console.log(user);
    ...
    dispatch(getManagerOrders({ queries: searchParams, userId: user._id }));
    const params = new URLSearchParams(window.location.search);
    const pageNumber = params.get("page");
    setPage(parseInt(pageNumber || 1));
  }, [searchParams]);
        ...
        <tbody>
          {orders.map((item, index) => (
            <tr key={index} className="border-b border-gray-200">
              ...
              <td className="p-4">{new Date(item.orderDate).toLocaleDateString()}</td>
